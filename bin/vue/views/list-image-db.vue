        ...
        <div style="text-align: left;padding-left: 20px;width: 95%;">
            ...
            <el-space :fill="true" wrap style="width:100%;">
                <el-card v-for="db in dbs.goodDbs" class="box-card" :key="db.id">
                    <template #header>
                        <div class="card-header">
                            <el-link class="bleft" :href="'/mag/image_db_detail/' + db.name" type="primary" title="查看数据库详情"
                            ...
                            <el-button-group class="ml-4 bright">
                                <el-button type="primary" @click="gotoSearch(db.name)">
                                    <el-icon>
                                        <Search />
                                    </el-icon>
                    ...
                    <el-descriptions class="margin-top" :column="2" :size="size" border>
                        ...
                        <el-descriptions-item>
                            ...
                            <el-text :type="db.buildingPipeline.currentOrd==2?'success':'info'" >
                ...
                <el-card v-for="db in dbs.brokenDbs" class="box-card" :key="db.id">
                    <template #header>
                        <div class="card-header">
                            <el-link class="bleft" :href="'/mag/imagedb_detail/' + db.name" type="primary" title="查看数据库详情"
                                style="font-size: 16pt;;">
                                <el-icon>
                                    <Coin />
                                </el-icon>
...
const ListImageDb = {
    ...
    methods: {
        ...
        gotoAdd() {
            ...
            location.href = ("/mag/alter_image_db")
            // this.$router.push({ path: '/alter_db' })
        },
        gotoAlter(db) {
            console.log("ready goto")
            location.href = ("/mag/alter_image_db/" + db)
            // this.$router.push({ path: '/alter_db/' + db })
        },
        testDelete(db) {
            this.$msgbox(
                {
                    message: '确定要删除本数据库吗？这将删除关联的所有图像和向量！',
                    title: '警告',
    ...
    template: "#list-image-db"
};

app.component("list-image-db", ListImageDb);
</script>
<style>
.bleft {
